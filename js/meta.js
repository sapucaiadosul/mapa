...
function adicionarAcao() {
    ...
    inputId.each(function (input) {
        ...
        if (inputId[input].name.match(/[0-9]+/) > maiorId)
            maiorId = inputId[input].name.match(/[0-9]+/);
    });
...
function aprovarAcao(label) {
    ...
    if ($(label).hasClass('disabled') == true)
        return false;
...
function atualizarIndicador(campo) {
    ...
    if (divisor > 0) {
        ...
        if (parseFloat($('#indicador_alcancado').val().replace(/\./g, '').replace(/\,/g, '.')) < referencia)
            $('#indicador_alcancado').val($('#indicador_referencia').val());
        ...
        if (parseFloat($('#indicador_alcancado').val().replace(/\./g, '').replace(/\,/g, '.')) < indicador)
            $('#indicador_alcancado').val($('#indicador_indicador').val());
    ...
    if ($('#indicador_alcancado_aprovado').length == 1) {
        ...
        if ($('#indicador_alcancado_aprovado:checked').length == 1 || campo == 'indicador_alcancado') {
            ...
            if (aprovado != aprovadoTotal || reprovado != reprovadoTotal)
        ...
        if (campo == 'indicador_referencia') {
            if (alcancado != alcancadoAnterior) {
                if ($('#indicador_alcancado_aprovado:checked').length == 1)
                    aprovado = alcancado - alcancadoAnterior;
                else if (alcancado > alcancadoAnterior) {
                    if (referencia > alcancadoAnterior)
                        alcancado = referencia;
                    else
                        alcancado = alcancadoAnterior;
            ...
            if (reprovadoTotal == 0 && alcancadoAnterior == alcancado) {
                if (aprovadoTotal > 0)
                    aprovado = aprovadoTotal + referenciaAnterior - referencia;
                if (aprovado < 0)
                    aprovado = 0;
...
function validarCamposFormulario(origem) {
    ...
    if ($('#numero').val().length == 0) {
        if (mensagem.length > 0)
            mensagem += ', ';
        mensagem += '<strong>Número da meta</strong>';
    ...
    if ($('#titulo').length > 0 && $('#responsavel').length > 0 && $('#data_inicial').length > 0 && $('#data_final').length > 0) {
        ...
        if ($('#responsavel').val().length == 0) {
            if (mensagem.length > 0)
                mensagem += ', ';
            mensagem += '<strong>Responsável</strong>';
            $('#responsavel').parent().addClass('has-error');
        } else {
            $('#responsavel').parent().removeClass('has-error');
        }

        if ($('#indicador_ods').val() == null || $('#indicador_ods').val().length === 0) {
            if (mensagem.length > 0)
                mensagem += ', ';
            mensagem += '<strong>Indicador ODS</strong>';
            $('#indicador_ods').parent().addClass('has-error');
        } else {
            $('#indicador_ods').parent().removeClass('has-error');
        }
    ...
    if (mensagem.length > 0)
        mensagem = 'Preencha o(s) campo(s): ' + mensagem + '.';

    return mensagem;
}

$(document).ready(function () {
    $('#indicador_ods').select2({
