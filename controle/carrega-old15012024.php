...
Class Carrega{
	...
	public function permissao(){
		...
		if(isset($_GET['mod'])){
			...
			$modulo = substr(Funcao::limpaInput($_GET['mod']),0,20);
			...
			if(!is_array($this->login->permissao($modulo))){
				...
				if(Funcao::carrega_arquivo('controle',$modulo)){
	...
	public function template(){
		...
		if(!is_array($login_retorno) || (isset($login_retorno[0]) && !is_string($login_retorno[0]))){
			...
			$this->mensagem.= Funcao::mensagem('Não foi possível acessar o sistema',MENSAGEM_ERRO);
			...
			if(isset($login_retorno[1]) && is_array($login_retorno[1]))
				$this->mensagem.= Funcao::mensagem(current($login_retorno[1]),key($login_retorno[1]));
		}
		...
		if(strlen($login_retorno[0]) > 0 && $this->login->alterarsenha == false){
			...
			if(isset($_GET['sair']) && strlen($this->mensagem) == 0)
				$this->mensagem.= Funcao::mensagem('Você saiu do sistema');
		...
		}else $this->mensagem.= Funcao::mensagem('Não foi possível carregar o sistema',MENSAGEM_ERRO);
	...
	private function excluir(){
		...
		if($retorno === true) return $this->mensagem.= Funcao::mensagem('Registro(s) excluído(s) com sucesso', MENSAGEM_SUCESSO);
		if($retorno === false) return $this->mensagem.= Funcao::mensagem('Não foi possível excluir o(s) registro(s)', MENSAGEM_ERRO);
		if(is_array($retorno)) return $this->mensagem.= Funcao::mensagem(current($retorno),key($retorno));
	...
	private function salvar(){
		...
		if($retorno !== false && is_numeric($retorno)){
			$this->mensagem.= Funcao::mensagem('Registro salvo com sucesso', MENSAGEM_SUCESSO);
		...
		}elseif($retorno === false) $this->mensagem.= Funcao::mensagem('Não foi possível salvar o registro', MENSAGEM_ERRO);
		elseif(!is_array($retorno) || count($retorno) != 2) $this->mensagem.= Funcao::mensagem('Não foi possível salvar o registro', MENSAGEM_ERRO);
		else{
			//Verifique se há campos incorretos
			if(is_array($retorno[0])) $GLOBALS['campo_erro'] = $retorno[0];
			//Verifica se há mensagem de retorno
			if(is_array($retorno[1])) $this->mensagem.= Funcao::mensagem(current($retorno[1]),key($retorno[1]));
		}
	...
	private function conteudo(){
		...
		if(is_null($this->modulo)){
			...
			elseif(count($retorno) >= 1){
				...
				if(count($retorno) == 2){
					if(is_array($retorno[1])) $this->mensagem.= Funcao::mensagem(current($retorno[1]),key($retorno[1]));
					elseif(is_string($retorno[1])) $this->mensagem.= Funcao::mensagem($retorno[1]);
				}
			}
		}
	...
	private function menu(){
		...
		if(!Funcao::carrega_arquivo('controle', 'menu')) return '';
		...
		if(!is_array($itens)){
			$this->mensagem.= Funcao::mensagem('Não foi possível carregar os itens de menu',MENSAGEM_ERRO);
			return '';
		}else{
			//Verifica se há erro à ser exibido
			if(count($itens) == 2){
				if(is_array($itens[1])) $this->mensagem.= Funcao::mensagem(current($itens[1]),key($itens[1]));
				elseif(is_string($itens[1])) $this->mensagem.= Funcao::mensagem($itens[1]);
				return '';
			}
		}
