...
Class MetaControle {
    ...
    function __construct($login, $banco) {
        //Carrega classe DAO
        if (!Funcao::carrega_arquivo('modelo', 'meta_dao'))
            return false;

        //Carrega classes base, pois na validação de campos o DAO é chamado depois
        if (!Funcao::carrega_arquivo('modelo', 'acao_class'))
            return false;
        if (!Funcao::carrega_arquivo('modelo', 'acompanhamento_class'))
            return false;
        if (!Funcao::carrega_arquivo('modelo', 'anexo_class'))
            return false;
        if (!Funcao::carrega_arquivo('modelo', 'meta_class'))
            return false;
    ...
    public function lista($pagina = 1) {
        ...
        $conteudo .= Funcao::exibirTotalRegistros($registros);

        //Paginação
        $conteudo .= Funcao::pagina_criar_links($this->modulo_arquivo, $paginas, $pagina);
...
//Template criado no código devido ao alto número de validações
            ...
            $linhas .= '<td class="text-center hidden-xs"><span class="badge alert-default"' . (isset($registro->indOds) ? ' data-toggle="tooltip" data-placement="bottom" title="' . $registro->indOds . '"' : '') . '>' . $registro->indOds . '</span></td>';
    ...
    private function carregarMapas($mapaId = null) {
        ...
        Funcao::carrega_arquivo('controle', 'mapa');
    ...
    private function carregarPerfis($responsavelId = null) {
        ...
        //Carrega classe de controle
        Funcao::carrega_arquivo('controle', 'perfil');
        //Cria objeto para utilizar funções públicas
