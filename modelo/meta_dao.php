...
Class MetaDAO {
    ...
    function __construct($login, $banco) {
        ...
        $this->funcao->carrega_arquivo('modelo', 'acao_class');
        $this->funcao->carrega_arquivo('modelo', 'acompanhamento_class');
        $this->funcao->carrega_arquivo('modelo', 'anexo_class');
        $this->funcao->carrega_arquivo('modelo', 'meta_class');
    ...
    private function atualizarMeta($registro) {
        ...
        if ($this->login->permissao($this->modulo_arquivo, EDITAR) >= TODOS) {//, ind_mon_valor, ind_mon_datahora
            ...
            $parametros[] = json_encode($registro->indOds);
    ...
    private function carregarFiltroPesquisa(&$requisicao) {
        ...
        if (is_numeric($ods)) {
            $requisicaoPesquisa .= ' and JSON_SEARCH(m.ind_ods, "one", ?) IS NOT NULL';
            $parametro[] = $ods;
        }
