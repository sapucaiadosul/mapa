...
Class Meta {
    ...
    function __construct(
        ...
        if (strlen($indObjetivo) > 0 && mb_strlen($indObjetivo) <= 300)
         ...
         if (isset($indOds) and !empty($indOds))
            $this->indOds = $indOds;
        ...
        $indValorAnterior = str_replace(',', '.', str_replace('.', '', (string)$indValorAnterior));
    ...
    public function __set($key, $value) {
        ...
        if ($key == 'dataInicial')
            if (strlen($value) == 10)
                $this->dataInicial = $this->funcao->limpar_data($value);
        if ($key == 'dataFinal')
            if (strlen($value) == 10)
                $this->dataFinal = $this->funcao->limpar_data($value);
        if ($key == 'dataConclusao')
            if (strlen($value) == 10)
                $this->dataConclusao = $this->funcao->limpar_data($value);
    }
