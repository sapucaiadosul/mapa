...
if (isset($registro) && get_class($registro) == 'Meta') {
    ...
    $valores_ods = @json_decode($registro->indOds, true);
    ?>
    <div class="col-xs-12">
            ...
            <fieldset class="col-xs-12 col-md-6 col-lg-7">
                <?php if ($this->login->permissao($this->modulo_arquivo, EDITAR) >= TODOS && $this->registro->desativado == false) { ?>
                    <div class="form-group">
                        ...
                        <input class="form-control" id="numero" maxlength="4" name="numero" onkeypress="return apenasNumeros()" type="text" value="<?php echo $this->registro->numero; ?>" required>
                    </div>
                <?php } ?>
                ...
                <?php if (($this->login->permissao($this->modulo_arquivo, EDITAR) >= TODOS || $this->permitirEdicao(isset($_GET['id']) ? $_GET['id'] : 0)) && $this->registro->desativado == false) { ?>
                    <?php if ($this->login->permissao($this->modulo_arquivo, EDITAR) >= TODOS && $this->registro->desativado == false) { ?>
                        ...
                        <div class="form-group col-xs-12 col-md-9 col-lg-8" style="padding-left: 10px;">
                            <label class="control-label" for="indicador_ods">Indicador ODS</label>
                            <select class="form-control" id="indicador_ods" name="indicador_ods[]" multiple="multiple">
                                <?php foreach ($objetivos as $valor => $objetivo): ?>
                                    <?php
                                    $selecionado = (in_array($valor, $valores_ods ?? []) && $valor != 0) ? 'selected' : '';
                                    ?>
